...
def find_one_line(filename: str):
    ...
    if len(find_list) > 1:
        ...
        index_element = int(
            input("Введите номер записи для редактирования: ")) - 1
        print(
            f"Для редактирования выбрана запись - {find_list[index_element]}")
...
def remove_data(filename: str):
    ...
    if len(find_list) > 0:
        ...
        if 0 <= index_for_remove < len(find_list):
            ...
            tel_book_lines.pop(tel_book_lines.index(
                find_list[index_for_remove]))
